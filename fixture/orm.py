...
class ORMFixture:
    ...
    @db_session
    def get_groups_not_with_contact(self, contact):
        orm_contact = list(select(c for c in ORMFixture.ORMContact if c.id == contact.id))[0]
        return self.convert_groups_to_model(
            list(select(g for g in ORMFixture.ORMGroup if orm_contact not in g.contacts)))

    @db_session
    def get_groups_with_any_contacts(self):
        return self.convert_groups_to_model(
            list(select(g for g in ORMFixture.ORMGroup if len(g.contacts) > 0)))
